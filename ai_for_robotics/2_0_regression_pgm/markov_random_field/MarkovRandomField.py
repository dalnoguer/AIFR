...
class ImageSegmenter(object):
    ...
    def singleton(self, i, j, label):
        #doneTODO: Calculate the singletone clique potential
        mean = self.label_mean[label]
        cov = np.diag(self.label_covariance[label])
        f = multivariate_normal(mean = mean,cov = cov)

        logprob = f.logpdf(self.features[i][j].reshape(1,3))

        potential = -1.0*logprob
        return potential

    def doubleton(self, i, j, label):
        potential = 0.0
        #doneTODO: Calculate the doubleton clique potential
        if i>0:
            if self.image_labels[i][j] == self.image_labels[i-1][j]:
        ...
        if i<self.image_height-1:
            if self.image_labels[i][j] == self.image_labels[i+1][j]:
                ...
                potential = potential + self.beta
        if j>0:
            if self.image_labels[i][j] == self.image_labels[i][j-1]:
                potential = potential - self.beta
            else:
                potential = potential + self.beta
    ...
    def calculateGlobalEnergy(self):
        ...
        singletons = 0.0
        doubletons = 0.0
        #doneTODO: Calculate the energy of the whole image
        for i in range(self.image_height):
            for j in range(self.image_width):
                energy += self.calculateLocalEnergy(i,j,self.image_labels[i][j])
        return energy

    def calculateLocalEnergy(self, i, j, label):
        energy = 0.0
        #doneTODO: Calculate the energy of one pixel
        energy = self.singleton(i,j,label)
        energy = energy + self.doubleton(i,j,label)
        return energy
